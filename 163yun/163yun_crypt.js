...
sjcl.cipher.aes = function(a) {
    ...
    for (b = 0; a; b++,
        a--)
        c = d[b & 3 ? a : a - 4],
            e[b] = 4 >= a || 4 > b ? c : g[0][f[c >>> 24]] ^ g[1][f[c >> 16 & 255]] ^ g[2][f[c >> 8 & 255]] ^ g[3][f[c & 255]]
}
;
...
function y(a, b, c) {
    ...
    var r = h[1]
        ...
        , x = h[4];
    for (m = 0; m < n; m++)
        h = a[e >>> 24] ^ r[f >> 16 & 255] ^ v[g >> 8 & 255] ^ w[b & 255] ^ d[p],
            l = a[f >>> 24] ^ r[g >> 16 & 255] ^ v[b >> 8 & 255] ^ w[e & 255] ^ d[p + 1],
            k = a[g >>> 24] ^ r[b >> 16 & 255] ^ v[e >> 8 & 255] ^ w[f & 255] ^ d[p + 2],
            b = a[b >>> 24] ^ r[e >> 16 & 255] ^ v[f >> 8 & 255] ^ w[g & 255] ^ d[p + 3],
            p += 4,
            e = h,
            f = l,
            g = k;
    for (m = 0; 4 > m; m++)
        s[c ? 3 & -m : m] = x[e >>> 24] << 24 ^ x[f >> 16 & 255] << 16 ^ x[g >> 8 & 255] << 8 ^ x[b & 255] ^ d[p++],
            h = e,
            e = f,
            f = g,
            g = b,
...
sjcl.hash.sha256 = function(a) {
    this.a[0] || this.D();
    a ? (this.q = a.q.slice(0),
        this.m = a.m.slice(0),
        this.g = a.g) : this.reset()
}
;
...
function z(a, b) {
    ...
    for (c = 0; 64 > c; c++)
        16 > c ? d = f[c] : (d = f[c + 1 & 15],
            e = f[c + 14 & 15],
            d = f[c & 15] = (d >>> 7 ^ d >>> 18 ^ d >>> 3 ^ d << 25 ^ d << 14) + (e >>> 17 ^ e >>> 19 ^ e >>> 10 ^ e << 15 ^ e << 13) + f[c & 15] + f[c + 9 & 15] | 0),
            ...
            n = k,
            k = l,
            l = d + (k & n ^ m & (k ^ n)) + (k >>> 2 ^ k >>> 13 ^ k >>> 22 ^ k << 30 ^ k << 19 ^ k << 10) | 0;
    g[0] = g[0] + l | 0;
    g[1] = g[1] + k | 0;
    g[2] = g[2] + n | 0;
    g[3] = g[3] + m | 0;
...
sjcl.mode.gcm = {
    ...
    decrypt: function(a, b, c, d, e) {
        ...
        e = e || 128;
        d = d || [];
        e <= h ? (b = g.bitSlice(f, h - e),
            f = g.bitSlice(f, 0, h - e)) : (b = f,
            f = []);
    ...
    n: function(a, b, c, d, e, f) {
        ...
        g = b.encrypt([0, 0, 0, 0]);
        96 === h ? (e = e.slice(0),
            e = r.concat(e, [1])) : (e = sjcl.mode.gcm.f(g, [0, 0, 0, 0], e),
            e = sjcl.mode.gcm.f(g, e, [0, 0, Math.floor(h / 0x100000000), h & 0xffffffff]));
...
sjcl.prng.prototype = {
    randomWords: function(a, b) {
        ...
        if (d & this.w) {
            ...
            for (g = 0; g < this.b.length && !(e = e.concat(this.b[g].finalize()),
                f += this.h[g],
                this.h[g] = 0,
            !d && this.F & 1 << g); g++)
                ;
            this.F >= 1 << this.b.length && (this.b.push(new sjcl.hash.sha256),
                this.h.push(0));
            this.c -= f;
            f > this.i && (this.i = f);
            this.F++;
            ...
            for (d = 0; 4 > d && !(this.e[d] = this.e[d] + 1 | 0,
                this.e[d]); d++)
                ;
        }
        for (d = 0; d < a; d += 4)
    ...
    addEntropy: function(a, b, c) {
        ...
        this.h[g] += b;
        this.c += b;
        h === this.l && (this.isReady() !== this.l && C("seeded", Math.max(this.i, this.c)),
...
function B(a) {
    for (var b = 0; 4 > b && !(a.e[b] = a.e[b] + 1 | 0,
        a.e[b]); b++)
        ;
...
sjcl.json = {
    ...
    encrypt: function(a, b, c, d) {
        ...
        "string" === typeof a && (g = sjcl.misc.cachedPbkdf2(a, f),
    ...
    decrypt: function(a, b, c, d) {
        ...
        "string" === typeof a && (f = sjcl.misc.cachedPbkdf2(a, b),
    ...
    encode: function(a) {
        ...
        for (b in a)
            if (a.hasOwnProperty(b))
                switch (b.match(/^[a-z0-9]+$/i) || q(new sjcl.exception.invalid("json encode: invalid property name")),
                    c += d + '"' + b + '":',
                    d = ",",
                    typeof a[b]) {
                    case "number":
    ...
    decode: function(a) {
        ...
        var b = {}, c, d;
        for (c = 0; c < a.length; c++)
            (d = a[c].match(/^(?:(["']?)([a-z][a-z0-9]*)\1):(?:(\d+)|"([a-z0-9+\/%*_.@=\-]*)")$/i)) || q(new sjcl.exception.invalid("json decode: this isn't json!")),
    ...
    d: function(a, b, c) {
        ...
        if (b === t)
            return a;
        for (var d in b)
            b.hasOwnProperty(d) && (c && (a[d] !== t && a[d] !== b[d]) && q(new sjcl.exception.invalid("required parameter overridden")),
                a[d] = b[d]);
        return a
    },
    X: function(a, b) {
...
var AES = {
    _$encrypt: function(e, t, n) {
        var e = sjcl.codec.utf8String.toBits(e)
            , t = sjcl.codec.utf8String.toBits(t)
            , a = new sjcl.cipher.aes(e)
            , n = n || localStorage.getItem("iv");
        n = sjcl.codec.utf8String.toBits(n);
        return sjcl.codec.base64.fromBits(sjcl.mode.ccm.encrypt(a, t, n))
    },
    _$decrypt: function(e, t, n) {
        var e = sjcl.codec.utf8String.toBits(e)
            , t = sjcl.codec.base64.toBits(t)
            , a = new sjcl.cipher.aes(e)
            , n = n || localStorage.getItem("iv");
        n = sjcl.codec.utf8String.toBits(n);
        try {
            return sjcl.codec.utf8String.fromBits(sjcl.mode.ccm.decrypt(a, t, n))
        } catch (i) {
            return
        }
    }
...
function encryptPassword(publicKey, md5_pass, iv) {
    return AES._$encrypt(publicKey, md5_pass, iv)
}

console.log(encryptPassword(
    "3/e2r4G94qS+1jlw",
    "736422d799c9f1a822dc38c3a87f3b3a",
    "AFFB6108A11F"
));
