...
function encrypt(password, iv, encryptKey) {
    T = new t(iv, "null", encryptKey);
    return n(T, password);
}

var B, z, O, I, P = 16, q = P, R = 65536, U = R >>> 1, K = R * R, H = R - 1;
l(38);  // 38 = 2 * maxdigits: rkey 那个接口里返回的 maxdigits: 固定值 19
...
function getUniqueTimestamp() {
    var s = new Date;
    return '' + s.getFullYear() + s.getMonth() + s.getDay() + s.getHours() + s.getSeconds() + s.getUTCMilliseconds()
}
